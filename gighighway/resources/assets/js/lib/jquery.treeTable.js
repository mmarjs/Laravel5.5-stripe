...
(function($) {
  ...
  $.fn.toggleBranch = function() {
      ...
      var start_SubString = elementId.indexOf('-') + 1;
      var end_SubString = elementId.length;

      var row_accountNumber = elementId.substr(start_SubString, end_SubString);

      if ($(this).hasClass("collapsed")) {
        $(this).expand();
        $("#expander-" + row_accountNumber).html("-");
    } else {
        $(this).collapse();
        $("#expander-" + row_accountNumber).html("+");
    }

      $(".child-of-node-" + row_accountNumber + " .expander").html("+");
      ...
      if(node.hasClass("parent")) {
        ...
        if(options.expandable) {
          //var newLink = '<a href="#" title="' + options.stringExpand + '" style="margin-left: -' + options.indent + 'px; padding-left: ' + options.indent + 'px" class="expander"></a>';
            //var arrow = '<div id="arrow_expander"><div class="expander"></div></div>';
            var arrow = '<div id="expander-' + cell[0].firstChild.nodeValue + '" class="expander">&#43;</div>';
          
          if(options.clickableNodeNames) {
              //cell.wrapInner(arrow+newLink);
              cell.html(arrow+cell.html());
          } else {
              //cell.html(arrow+newLink+cell.html());
              cell.html(arrow+cell.html());
          }

          $(cell[0].firstChild).unbind("click").bind("click", function () {
              node.toggleBranch();
              return false;
          }).mousedown(function () {
              return false;
...
$(document).ready(function () {
    $("#tblMembers3x3DetailsTreeTable").treeTable({ expandable: true, indent: 13, initialState: "collapsed", treeColumn: 0, clickableNodeNames: true });
});